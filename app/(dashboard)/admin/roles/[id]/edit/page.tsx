...
export default function EditRolePage() {
  ...
  const loadRoleData = async () => {
    try {
      ...
      const [roleData, permissionsResponse] = await Promise.all([
        roleService.getRoleById(roleId),
        permissionService.getPermissions({ limit: 100 })
      ])
      ...
      // Mark permissions as selected based on role's current permissions
      const rolePermissionIds = new Set(
        roleData.permissions?.map((p: any) => typeof p === 'string' ? p : p._id || p.id) || []
      );
      const permissionsWithSelection = permissionsResponse.items.map(permission => ({
        ...permission,
        selected: rolePermissionIds.has((permission as any)._id || permission.id)
      }))
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="text-sm text-blue-800">
                    <div className="font-medium">Role Information</div>
                    <div>Created: {new Date(role.createdAt).toLocaleDateString()}</div>
                    <div>Users with this role: {role.userCount}</div>
