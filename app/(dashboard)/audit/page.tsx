...
import { TwoFactorTokenDialog } from "@/components/admin/security/two-factor-token-dialog";
...
const AuditLogDashboard: React.FC<AuditLogDashboardProps> = ({
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {overview && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {overview.criticalEvents.toLocaleString()}
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {overview.failedOperations.toLocaleString()}
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {overview.successfulOperations > 0
                  ? Math.round(
                      (overview.successfulOperations / overview.totalEvents) *
                        100
                    )
                  : 0}
                %
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            by {log.user?.firstName || "Unknown"} on{" "}
                            {log.resource}
                          </p>
                        </div>
                      </div>
                  ...
                  {securityEvents.length > 0 ? (
                    <div className="space-y-2">
                      {securityEvents.map((event, index) => (
                        <div
                          key={event.id || `security-event-${index}`}
                        ...
                        >
                          <div className="flex items-center space-x-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {event.user?.firstName || "Unknown"} from{" "}
                                {event.ipAddress}
                              </p>
                            </div>
                          </div>
                  ...
                  {failedOperations.length > 0 ? (
                    <div className="space-y-2">
                      {failedOperations.map((operation, index) => (
                        <div
                          key={operation.id || `failed-op-${index}`}
                          className="flex items-center justify-between p-2 border rounded"
                        >
                          <div className="flex items-center space-x-2">
              ...
              {realTimeEvents.length > 0 ? (
                <div className="space-y-1">
                  {realTimeEvents.map((event, index) => (
                    <div
                      key={index}
                      className="text-xs p-1 border-l-2 border-blue-500 pl-2"
                    >
      ...
      <TwoFactorTokenDialog
        open={twoFactorDialog.open}
        onOpenChange={(open) =>
          setTwoFactorDialog((prev) => ({ ...prev, open }))
        }
        onSuccess={executeExport}
        title="Export Audit Logs - 2FA Required"
        description={`Please verify your identity to export audit logs in ${twoFactorDialog.format.toUpperCase()} format`}
        action="Export"
      />
    </div>
  );
