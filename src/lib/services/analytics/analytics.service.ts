...
class AnalyticsService {
  ...
  private async processBatch(): Promise<void> {
    ...
    try {
      // Transform events to match backend API schema
      const transformedEvents = eventsToSend.map(event => ({
        type: event.eventType, // Backend expects 'type' not 'eventType'
        userId: undefined, // Will be set by backend from auth token
      ...
      if (!response.ok) {
        ...
        console.error('❌ Batch tracking failed:', {
          status: response.status,
          statusText: response.statusText,
          error: errorData
        });
        throw new Error(`Batch tracking failed: ${response.status} - ${JSON.stringify(errorData)}`);
      }
