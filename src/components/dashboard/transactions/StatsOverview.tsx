    ...
    const statCards = [
        {
            ...
            format: 'currency',
            icon: DollarSign,
            description: (stats?.totalAmount ?? 0) > 0 ? 'All time revenue' : 'No transactions yet',
            trend: stats?.totalAmount && stats.totalAmount > 0 ? '+' : '',
        },
        {
            title: 'Net Earnings',
            value: stats?.totalNet || 0,
            format: 'currency',
            icon: TrendingUp,
            description: stats?.totalFees ? `After $${stats.totalFees.toFixed(2)} fees` : 'After fees deduction',
            highlight: true,
        },
        {
        ...
        },
        {
            ...
            format: 'number',
            icon: RefreshCw,
            description: (stats?.totalTransactions ?? 0) > 0 ? 'All payment attempts' : 'No transactions',
        },
    ];
                ...
                <Card key={index} className={card.highlight ? 'border-primary' : ''}>
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${card.highlight ? 'text-primary' : ''}`}>
                            {card.trend && <span className="text-green-500 text-sm mr-1">{card.trend}</span>}
